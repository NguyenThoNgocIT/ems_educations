...
public class AutoScheduleService {
    ...
    public int generateScheduleForCourseClass(UUID courseClassId, UUID instructorId) {
        ...
        return generateScheduleForCourseClass(courseClass, semester, rooms, timeSlots, selectedInstructor, true);
    }
    ...
    private int generateScheduleForCourseClass(
            ...
            boolean strict) {
        deactivateLegacyAutoGeneratedRecurringSchedules(courseClass.getCourseClassId());
        ...
        int created = createFixedWeeklySessions(courseClass, semester, instructor, rooms, timeSlots, remainingPeriods);
        if (strict && created == 0) {
    ...
    private int resolveSessionPeriods(int remainingPeriods) {
        return Math.max(1, Math.min(3, remainingPeriods));
    }

    private void deactivateLegacyAutoGeneratedRecurringSchedules(UUID courseClassId) {
        List<Schedule> legacyPatterns = scheduleRepository.findActiveRecurringByCourseClass(courseClassId);
        if (legacyPatterns.isEmpty()) {
            return;
        }
        LocalDateTime now = LocalDateTime.now();
        for (Schedule schedule : legacyPatterns) {
            schedule.setIsActive(false);
            schedule.setDeletedAt(now);
            schedule.setStatus("REPLACED_BY_DATED_SCHEDULE");
            schedule.setNote("Da vo hieu hoa mau lich tuan cu de xep lich goc theo tung buoi");
        }
        scheduleRepository.saveAll(legacyPatterns);
    }
