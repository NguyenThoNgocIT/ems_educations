...
public interface UserRoleRepository extends JpaRepository<UserRole, UserRoleId> {
    ...
    @Query("SELECT ur FROM UserRole ur JOIN FETCH ur.role WHERE ur.id.userId IN :userIds AND ur.isActive = true")
    List<UserRole> findActiveRolesByUserIds(@Param("userIds") Collection<UUID> userIds);
