...
class AutoScheduleServiceTest {
    ...
    void generateScheduleForSemester_createsFixedSessionsByCourseCredits() {
        ...
        when(scheduleRepository.findActiveAutoGeneratedByCourseClass(courseClassId)).thenReturn(List.of());
        when(scheduleRepository.sumActivePeriodsByCourseClass(courseClassId)).thenReturn(0L);
        ...
        when(courseRegistrationRepository.countByCourseClassIdAndIsActiveTrue(courseClassId)).thenReturn(25L);
        when(employeeLeaveRequestRepository.hasApprovedLeaveOnDate(eq(instructorId), any())).thenReturn(false);
        when(scheduleRepository.hasRecurringRoomTimeOverlap(eq(room.getRoomId()), eq(semesterId), any(), eq(timeSlot.getStartTime()), eq(timeSlot.getEndTime()))).thenReturn(false);
        when(scheduleRepository.hasRecurringInstructorTimeOverlap(eq(instructorId), eq(semesterId), any(), eq(timeSlot.getStartTime()), eq(timeSlot.getEndTime()))).thenReturn(false);
        when(scheduleRepository.hasRecurringCourseClassTimeOverlap(eq(courseClassId), eq(semesterId), any(), eq(timeSlot.getStartTime()), eq(timeSlot.getEndTime()))).thenReturn(false);
        when(scheduleRepository.hasRoomTimeOverlap(eq(room.getRoomId()), any(), eq(timeSlot.getStartTime()), eq(timeSlot.getEndTime()))).thenReturn(false);
        when(overrideRepository.hasRoomTimeOverlap(eq(room.getRoomId()), any(), eq(timeSlot.getStartTime()), eq(timeSlot.getEndTime()))).thenReturn(false);
        when(scheduleRepository.hasInstructorTimeOverlap(eq(instructorId), any(), eq(timeSlot.getStartTime()), eq(timeSlot.getEndTime()), eq(null))).thenReturn(false);
        when(overrideRepository.hasInstructorTimeOverlap(eq(instructorId), any(), eq(timeSlot.getStartTime()), eq(timeSlot.getEndTime()))).thenReturn(false);
        when(scheduleRepository.hasCourseClassTimeOverlap(eq(courseClassId), any(), eq(timeSlot.getStartTime()), eq(timeSlot.getEndTime()))).thenReturn(false);
        when(overrideRepository.hasCourseClassTimeOverlap(eq(courseClassId), any(), eq(timeSlot.getStartTime()), eq(timeSlot.getEndTime()))).thenReturn(false);
        when(courseClassRepository.findById(courseClassId)).thenReturn(Optional.of(courseClass));
        when(scheduleRepository.findByCourseClassCourseClassId(courseClassId)).thenReturn(List.of());
    ...
    @Test
    void generateScheduleForCourseClass_regeneratesThreeCreditClassWithMultipleSessionsPerWeek() {
        UUID semesterId = UUID.randomUUID();
        UUID courseClassId = UUID.randomUUID();
        UUID instructorId = UUID.randomUUID();
        ...
        CourseClass courseClass = new CourseClass();
        courseClass.setCourseClassId(courseClassId);
        courseClass.setClassCode("CNTT-K24-02");
        courseClass.setCourseId(course.getCourseId());
        courseClass.setCourse(course);
        courseClass.setSemesterId(semesterId);
        courseClass.setMaxStudent(40);
        ...
        when(timeSlotRepository.findAll()).thenReturn(List.of(timeSlot));
        when(employeeRepository.findById(instructorId)).thenReturn(Optional.of(instructor));
        when(scheduleRepository.deactivateActiveSchedulesByCourseClass(eq(courseClassId), any())).thenReturn(12);
        when(scheduleRepository.sumActivePeriodsByCourseClass(courseClassId)).thenReturn(0L);
        when(courseRegistrationRepository.countByCourseClassIdAndIsActiveTrue(courseClassId)).thenReturn(25L);
        when(employeeLeaveRequestRepository.hasApprovedLeaveOnDate(eq(instructorId), any())).thenReturn(false);
        when(scheduleRepository.hasRecurringRoomTimeOverlap(eq(room.getRoomId()), eq(semesterId), any(), eq(timeSlot.getStartTime()), eq(timeSlot.getEndTime()))).thenReturn(false);
        ...
        int created = service.generateScheduleForCourseClass(courseClassId, instructorId);

        ArgumentCaptor<Schedule> captor = ArgumentCaptor.forClass(Schedule.class);
        verify(scheduleRepository, atLeastOnce()).save(captor.capture());

        List<Schedule> generated = captor.getAllValues();
        assertThat(created).isEqualTo(15);
        verify(scheduleRepository).deactivateActiveSchedulesByCourseClass(eq(courseClassId), any());
        assertThat(generated).hasSize(15);
        assertThat(generated).allSatisfy(schedule -> assertThat(schedule.getNumberOfPeriods()).isEqualTo(3));
        assertThat(generated.stream().filter(s -> !s.getDate().isAfter(startDate.plusDays(6))).count()).isEqualTo(2);
        assertThat(generated).noneSatisfy(schedule -> assertThat(schedule.getDate().getDayOfWeek()).isEqualTo(DayOfWeek.SUNDAY));
        assertThat(generated.stream().map(Schedule::getDate).distinct().count()).isGreaterThan(8);
    }
}
