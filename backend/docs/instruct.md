...
- **Audit:** Luôn đảm bảo các trường `CreatedAt`, `UpdatedAt`, `IsDeleted` được xử lý tự động qua Base Entity, SoftDeleteEntity.
...
- Xử lý lỗi tập trung qua `GlobalExceptionHandler`. Không sử dụng `try-catch` bừa bãi trong Controller.
...
- và hiển thị thông báo: *"Không thể thay đổi Mã định danh và Email sau khi đăng ký"*. 
