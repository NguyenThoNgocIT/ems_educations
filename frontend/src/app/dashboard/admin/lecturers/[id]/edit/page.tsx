...
const mockLecturerData: Record<string, LecturerFormData> = {
  '1': {
...
export default function EditLecturerPage() {
  ...
  useEffect(() => {
    if (id) {
      // TODO: Gọi API lấy chi tiết giảng viên
      setTimeout(() => {
        const data = mockLecturerData[id] || {
          instructorCode: 'GV000',
          employeeCode: 'NV000',
          departmentId: 'Khoa CNTT',
          degreeId: 'Thạc sĩ',
          isActive: true
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    // TODO: Gọi API cập nhật giảng viên
    setTimeout(() => {
      setLoading(false);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Chỉnh sửa giảng viên</CardTitle>
        </CardHeader>
        <CardContent>
