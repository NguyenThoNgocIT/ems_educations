...
const AddJobCategoryModal = ({ isOpen, onClose }: AddJobCategoryModalProps) => {
  if (!isOpen) return null;

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    alert("🚀 Mona System: Đã thêm danh mục công việc mới thành công!");
    onClose();
  };
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          <div className="space-y-1.5">
            ...
            <input
              required
              type="text"
              placeholder="Ví dụ: Kỹ sư phần mềm, Nội trợ..."
