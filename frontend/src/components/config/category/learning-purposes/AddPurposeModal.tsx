...
const AddPurposeModal = ({ isOpen, onClose }: AddPurposeModalProps) => {
  if (!isOpen) return null;

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    alert("🚀 Hệ thống: Đã thêm mục đích học tập mới thành công!");
    onClose();
  };
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          <div className="space-y-1.5">
            <label className="ml-1 text-[11px] font-bold tracking-widest text-slate-400">
              Tên mục đích / Mục tiêu học
            </label>
            <input
              required
              type="text"
              placeholder="Ví dụ: Lấy bằng PTE, Phỏng vấn xin việc..."
