...
const AddRejectionModal = ({ isOpen, onClose }: AddRejectionModalProps) => {
  if (!isOpen) return null;

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    alert(
      "🚀 Mona System: Đã cập nhật lý do từ chối mới vào danh sách hệ thống!",
    );
    onClose();
  };
      ...
      <div className="animate-in zoom-in w-full max-w-lg rounded-[32px] bg-white p-8 shadow-2xl duration-300 dark:bg-slate-900">
        <div className="mb-8 flex items-center justify-between border-b border-slate-50 pb-4 dark:border-slate-700">
          <div className="flex items-center gap-3 text-slate-900 dark:text-white">
            <div className="rounded-xl bg-rose-50 p-2 text-rose-600 dark:bg-rose-900/30">
              <FileX2 size={20} />
            </div>
        ...
        <form className="space-y-6" onSubmit={handleSubmit}>
          <div className="space-y-1.5">
            ...
            <input
              required
              type="text"
              placeholder="Ví dụ: Đã học trung tâm khác..."
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              rows={3}
              placeholder="Nhập diễn giải lý do..."
