...
import { initialHolidayData, Holiday } from "./holidayData";
...
const HolidayManagement = () => {
  ...
  // --- LOGIC TÌM KIẾM THEO TÊN NGÀY NGHỈ ---
  const filteredData = useMemo(() => {
    return initialHolidayData.filter((item) =>
    ...
    <div className="overflow-hidden rounded-[32px] border border-slate-100 bg-white font-sans shadow-sm dark:border-slate-700 dark:bg-slate-900">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b p-6 dark:border-slate-700">
        <div className="flex max-w-md flex-1 items-center gap-3">
          <div className="relative w-full">
            <input
              type="text"
              placeholder="Tìm tên ngày nghỉ hoặc sự kiện..."
              value={searchQuery}
        ...
        >
          <Plus size={18} /> Thêm ngày nghỉ mới
        </button>
      </div>
        ...
        <table className="w-full min-w-[1100px] border-collapse text-left text-[13px]">
          <thead>
            <tr className="border-b bg-slate-50/30 text-sm font-extrabold tracking-wider text-slate-900 dark:border-slate-700 dark:text-white">
              <th className="px-6 py-5">Tên sự kiện ngày nghỉ</th>
              ...
              <th className="px-6 py-5">Đối tượng áp dụng</th>
            ...
            {filteredData.length > 0 ? (
              filteredData.map((item) => (
                ...
                >
                  ...
                  <td className="px-6 py-6">
                    <span
                      className={`rounded-lg px-3 py-1 text-[10px] font-bold text-white shadow-sm ${item.type === "Nghỉ lễ" ? "bg-purple-500" : "bg-amber-500"}`}
                    >
                      {item.type}
                    </span>
                  </td>
                  <td className="px-6 py-6 text-center font-bold tracking-normal text-indigo-700 italic">
                    {item.start}
                  </td>
                  <td className="px-6 py-6 text-center font-bold tracking-normal text-rose-500 italic">
                    {item.end}
                  </td>
                  <td className="px-6 py-6">
                  ...
                  <td className="px-6 py-6 text-center">
                    <span
                      className={`rounded-lg px-3 py-1.5 text-[10px] font-bold text-white italic shadow-sm ${item.status === "Đang áp dụng" ? "bg-emerald-500" : "bg-blue-500"}`}
                    >
                      {item.status}
                    </span>
                  </td>
      ...
      <div className="flex items-center justify-end border-t bg-slate-50/10 p-5 dark:border-slate-700">
        <span className="mr-6 text-[11px] font-bold tracking-widest text-slate-600">
          Tổng cộng: {filteredData.length} sự kiện
        </span>
