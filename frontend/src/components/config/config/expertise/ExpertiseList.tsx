        ...
        <table className="w-full min-w-[1000px] border-collapse text-left text-[13px]">
          <thead>
            <tr className="border-b bg-slate-50/30 text-sm font-extrabold tracking-wider text-slate-900 dark:border-slate-700 dark:text-white">
              <th className="px-8 py-5">Mã chuyên môn</th>
              <th className="px-6 py-5">Tên chuyên môn</th>
              <th className="px-6 py-5">Ngày khởi tạo</th>
          ...
          <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
            {filteredData.length > 0 ? (
              filteredData.map((item, index) => (
                ...
                >
                  <td className="px-8 py-6 font-bold text-slate-900 italic dark:text-slate-300">
                    {item.id}
                  </td>
