...
import { initialTuitionData, TuitionPackage } from "./tuitionData";
...
const TuitionPackages = () => {
  ...
  const filteredData = useMemo(() => {
    return initialTuitionData.filter((item) =>
    ...
    <div className="overflow-hidden rounded-[32px] border border-slate-100 bg-white font-sans shadow-sm dark:border-slate-700 dark:bg-slate-900">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b p-6 dark:border-slate-700">
        <div className="flex max-w-md flex-1 items-center gap-3">
          <div className="relative w-full">
            <input
              type="text"
              placeholder="Tìm kiếm mã gói học phí..."
              value={searchQuery}
        ...
        <table className="w-full min-w-[1000px] border-collapse text-left text-[13px]">
          <thead>
            <tr className="border-b bg-slate-50/30 text-sm font-extrabold tracking-wider text-slate-900 dark:border-slate-700 dark:text-white">
              ...
              <th className="px-6 py-4">Loại chiết khấu</th>
              <th className="px-6 py-4 text-right">Giá trị giảm</th>
          ...
          <tbody className="divide-y divide-slate-50 text-slate-900 dark:divide-slate-800 dark:text-slate-300">
            {filteredData.length > 0 ? (
              filteredData.map((item, index) => (
                ...
                >
                  <td className="px-6 py-6 font-bold text-slate-900 italic dark:text-slate-200">
                    {item.id}
                  </td>
                  <td className="bg-blue-50/20 px-6 py-6 text-center font-bold text-indigo-700">
                    {item.duration}
                  </td>
                  <td className="px-6 py-6">
                    <span className="rounded-lg bg-blue-500 px-3 py-1 text-[10px] font-bold text-white shadow-sm">
                      {item.type}
                    </span>
                  </td>
                  <td className="px-6 py-6 text-right text-base leading-relaxed font-bold text-rose-500 italic">
              ...
              <tr>
                ...
                >
                  Không tìm thấy gói học phí nào...
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="flex items-center justify-end border-t bg-slate-50/10 p-5 dark:border-slate-700">
        <span className="mr-6 text-[11px] font-bold tracking-widest text-slate-600">
          Tổng cộng: {filteredData.length} gói
        </span>
