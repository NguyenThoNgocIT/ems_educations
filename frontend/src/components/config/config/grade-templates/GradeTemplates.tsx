...
import { gradeTemplateData } from "./gradeTemplateData";
import AddGradeTemplateModal from "./AddGradeTemplateModal";
...
const GradeTemplates = () => {
  ...
  const filteredData = useMemo(() => {
    return gradeTemplateData.filter((item) =>
    ...
    <div className="overflow-hidden rounded-[32px] border border-slate-100 bg-white font-sans shadow-sm dark:border-slate-700 dark:bg-slate-900">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b p-6 dark:border-slate-700">
        <div className="flex flex-1 items-center gap-3">
          ...
          <div className="relative w-full max-w-sm">
            <input
              type="text"
              placeholder="Tìm kiếm mẫu bảng điểm..."
              value={searchQuery}
        ...
        <table className="w-full min-w-[800px] border-collapse text-left">
          <thead>
            <tr className="border-b bg-slate-50/30 text-sm font-extrabold tracking-wider text-slate-900 dark:border-slate-700 dark:text-white">
              <th className="px-8 py-5">Tên biểu mẫu bảng điểm</th>
              <th className="px-6 py-5 text-center">Người khởi tạo</th>
          ...
          <tbody className="divide-y divide-slate-50 text-[13px] dark:divide-slate-800">
            {filteredData.length > 0 ? (
              filteredData.map((item) => (
                ...
                >
                  <td className="cursor-pointer px-8 py-6 text-sm font-bold text-indigo-700 italic hover:underline">
                    {item.name}
                  </td>
                  ...
                  <td className="px-8 py-6">
                    <div className="flex items-center justify-center gap-5">
                      ...
                      <button
                        title="Xóa mẫu"
                        className="text-rose-500 opacity-40 transition-transform hover:scale-125 hover:opacity-100"
                      >
                        <Trash2 size={18} />
                      </button>
                    </div>
              ...
              <tr>
                ...
                >
                  Không tìm thấy mẫu bảng điểm nào...
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="flex items-center justify-end border-t bg-slate-50/10 p-5 dark:border-slate-700">
        <span className="mr-6 text-[11px] font-bold text-slate-600">
          Tổng số: {filteredData.length} mẫu
        </span>
