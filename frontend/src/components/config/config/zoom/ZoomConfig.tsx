...
import { initialZoomData } from "./zoomData";
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-50 p-6 dark:border-slate-700">
        <div className="flex max-w-md flex-1 items-center gap-3">
          <div className="relative w-full">
            <input
              type="text"
              placeholder="Tìm email hoặc Account ID..."
              value={searchQuery}
        ...
        >
          <Plus size={18} /> Thêm mới tài khoản
        </button>
      </div>
        ...
        <table className="w-full min-w-[1200px] border-collapse text-left text-[13px]">
          <thead>
            <tr className="border-b bg-slate-50/30 text-sm font-extrabold tracking-wider text-slate-900 dark:border-slate-700 dark:text-white">
              <th className="px-8 py-5">Email đăng ký</th>
              <th className="px-6 py-5">Account ID</th>
              <th className="px-6 py-5">Client ID</th>
              <th className="px-6 py-5">Client Secret</th>
            ...
            {filteredData.length > 0 ? (
              filteredData.map((item, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-6 font-mono text-[11px] font-bold text-slate-600">
                    {item.accountId}
                  </td>
                  <td className="px-6 py-6 font-mono text-[11px] font-bold text-slate-600">
                    {item.clientId}
                  </td>
                  <td className="max-w-[150px] truncate px-6 py-6 font-bold text-slate-400 italic">
                    {item.clientSecret}
                  </td>
                  <td className="px-6 py-6 text-center">
                    <span
                      className={`rounded-lg px-3 py-1 text-[10px] font-bold text-white shadow-sm ${item.status === "Đang chạy" ? "bg-emerald-500" : "bg-rose-500"}`}
                    >
                      {item.status}
                    </span>
                  </td>
                  ...
                  <td className="px-6 py-6 font-bold text-slate-900 underline decoration-gray-200 dark:text-slate-200">
                    {item.creator}
                  </td>
              ...
              <tr>
                ...
                >
                  Không tìm thấy tài khoản Zoom nào...
                </td>
              </tr>
            )}
          </tbody>
