...
import { initialShiftData, Shift } from "./shiftData";
import AddShiftModal from "./AddShiftModal";

const ShiftManagement = () => {
  const [shifts] = useState<Shift[]>(initialShiftData);
  const [searchQuery, setSearchQuery] = useState("");
  const [isModalOpen, setIsModalOpen] = useState(false);

  // --- LOGIC TÌM KIẾM THEO TÊN CA ---
    ...
    <div className="overflow-hidden rounded-[32px] border border-slate-100 bg-white font-sans shadow-sm dark:border-slate-700 dark:bg-slate-900">
        ...
        <table className="w-full min-w-[800px] border-collapse text-left text-[13px]">
          <thead>
            <tr className="border-b bg-slate-50/30 text-sm font-extrabold tracking-wider text-slate-900 dark:border-slate-700 dark:text-white">
              <th className="px-8 py-5">Tên ca học hiển thị</th>
              <th className="px-6 py-5 text-center">Thời lượng</th>
              <th className="px-6 py-5 text-center">Giờ bắt đầu</th>
              <th className="px-6 py-5 text-center">Giờ kết thúc</th>
              ...
              filteredShifts.map((item) => (
                ...
                >
                  ...
                  <td className="px-6 py-6 text-center font-bold">
                    <span className="rounded-md bg-slate-50 px-3 py-1 dark:bg-slate-800">
                      {item.duration} phút
                    </span>
                  </td>
      ...
      <div className="flex items-center justify-end border-t bg-slate-50/10 p-5 dark:border-slate-700">
        <span className="mr-6 text-[11px] font-bold tracking-widest text-slate-600">
          Tổng: {filteredShifts.length} ca học
        </span>
