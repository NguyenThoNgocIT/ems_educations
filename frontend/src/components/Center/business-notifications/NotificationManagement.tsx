    ...
    <div className="min-h-screen font-sans">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="flex flex-col items-center justify-between gap-4 rounded-xl border border-slate-100 bg-white p-6 shadow-sm md:flex-row dark:border-slate-700 dark:bg-slate-900">
          <h1 className="text-xl font-bold tracking-tight text-slate-900 dark:text-white leading-snug leading-snug">
            Lịch sử thông báo
          </h1>
          <button
            onClick={() => setIsModalOpen(true)}
            className="flex items-center gap-2 rounded-lg bg-green-500 px-4 py-2 font-bold text-white shadow-md shadow-green-500/20 transition-all hover:bg-green-600"
          >
            ...
            <table className="w-full border-collapse text-left">
              <thead>
                <tr className="border-b bg-slate-50 dark:border-slate-700 dark:bg-slate-800/50">
                  <th className="w-16 p-4 text-center text-xs font-bold tracking-wider text-slate-600">
                    #
                  </th>
                  <th className="p-4 text-xs font-bold tracking-wider text-slate-600">
                    Ngày gửi thông báo
                  </th>
                  <th className="p-4 text-xs font-bold tracking-wider text-slate-600">
                    Tiêu đề
                  ...
                  <th className="p-4 text-center text-xs font-bold tracking-wider text-slate-600">
                    Chức năng
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y dark:divide-slate-800">
                {notificationsData.map((item) => (
                  ...
                  >
                    ...
                    <td className="p-4 text-sm font-bold text-slate-900 dark:text-slate-2 leading-relaxed00">
                      {item.sendDate}
                    </td>
                    <td className="p-4 text-sm leading-snug text-slate-600 dark:text-slate-4 leading-relaxed00">
                      {item.title}
                    </td>
